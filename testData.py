...
def getData(path=pwr_path, force_reload=False):
    ...
    try:
        print("[INFO] Checking for raw data...")
        raw_data = pd.read_pickle("pkl files/raw_data.pkl")
        if force_reload:
            raise Exception("Force Reload is True.")
        else:
            print("[INFO] Raw data file found, loading data...")
    except: 
        print("[INFO] Raw data not found...")
        ...
        with open("pkl files/lenDict.pkl", "wb") as f:
            pickle.dump(lenDict, f)
        raw_data.to_pickle("pkl files/raw_data_backup.pkl")
    finally:
        return raw_data

data = getData(force_reload=True)
